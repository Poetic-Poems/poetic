...
#
# Flags a pull request that rewrites an *open* tech-debt item's body while
# leaving its status: untouched. DIR defaults to tech-debt.
#
# Lifecycle edits to a register item are meant to be frontmatter-only (see
# docs/TECH-DEBT-REGISTER.md): claiming flips status: to in-progress,
# resolving flips it to resolved and fills resolved:/ref:. The atomic
# reservation reserve-tech-debt-id.pl provides (see its own header) closes
# the id-allocation half of the collision this register saw on 2026-08-08
# (agent-ops PR #257), but not the other half: a writer whose local clone
# already contains an item that has since been resolved on the far side
# can still overwrite its body as an ordinary content *modification* --
# git records that as a change to an existing file, not the add/add
# conflict a same-id *filing* collision produces, so neither the
# deletion/rename guard nor td-check.pl notices. An open item whose body
# changed without its status: field moving is exactly that failure mode
# (or, short of a race, a lifecycle-editing policy breach) -- either way,
# a reviewer should see a red check rather than a silently corrupted
# permanent record.
#
# For each file present (and parseable) at both refs under DIR whose body
# differs, and whose status: at BASE_REF is "open", and whose status: at
# HEAD_REF is unchanged from BASE_REF: report it as a problem. Prints one
# line per problem; exits 0 when none are found, 1 when problems were
# found, >1 on usage or I/O error.
#
