...
- **Browser-safe renderer** — the `poetic/browser` library export renders poems to HTML in plain JavaScript (no filesystem), enabling live-preview web apps and server-side rendering (see [`docs/RENDERER-BROWSER.md`](docs/RENDERER-BROWSER.md))
...
Each notable change records its own entry under a `## Changelog` heading in its pull request's description, not in `CHANGELOG.md` directly (see AGENTS.md's "Documentation principles"). A release pull request assembles those descriptions into the file first — `scripts/assemble-changelog.sh` folds every merged pull request's `## Changelog` section into `[Unreleased]` — then renames `[Unreleased]` to the new `## [X.Y.Z]` heading alongside the version bump. The first assembly after adoption has no progress marker to resume from yet, so it takes `--since <commit>`, naming the commit at which this changelog convention itself landed; every run after that reads and rewrites the `<!-- changelog:assembled-through sha=… -->` marker the assembler keeps near the top of the file, so a later run needs no `--since`.
